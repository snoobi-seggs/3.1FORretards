...
public final class EquipCommand implements CommandHandler {
    ...
    public void execute(Player sender, Player targetPlayer, List<String> args) {
		...
		*/
		...
		Scene scene = targetPlayer.getScene();
		GadgetData gadgetData = GameData.getGadgetDataMap().get(itemId);
		...
		if (gadgetData != null) {
			ItemData weaponItemData = currentWeapon.getItemData();
			weaponItemData.setGadgetId(itemId);
			currentWeapon.setItemData(weaponItemData);
			//currentWeapon.setItemId();
			
			scene.broadcastPacket(new PacketAvatarChangeCostumeNotify(targetPlayer.getTeamManager().getCurrentAvatarEntity()));
			
			CommandHandler.sendMessage(sender,"item data = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemData() + "\nitem data gadget id = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemData().getGadgetId() + "\nweapon item id = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemId());
		} else if (itemData != null) {
			ItemData weaponItemData = currentWeapon.getItemData();
			//String itemIdStr = itemId.toString();
			weaponItemData.setGadgetId(Integer.parseInt("500" + itemId));
			currentWeapon.setItemData(weaponItemData);
			currentWeapon.setItemId(itemId);
			
			scene.broadcastPacket(new PacketAvatarChangeCostumeNotify(targetPlayer.getTeamManager().getCurrentAvatarEntity()));
			CommandHandler.sendMessage(sender,"item data = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemData() + "\nitem data gadget id = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemData().getGadgetId() + "\nweapon item id = " + targetPlayer.getTeamManager().getCurrentAvatarEntity().getAvatar().getWeapon().getItemId());
		}
		
		//CommandHandler.sendMessage(sender,"costumeId = " + currentAvatar.getCostume());
        /*if (gadgetData != null) {
