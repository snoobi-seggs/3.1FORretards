...
import emu.grasscutter.command.pojo.AvatarInfoListItem;
import emu.grasscutter.command.pojo.EquipListItem;
import emu.grasscutter.command.pojo.Response;
    ...
    public void execute(Player sender, Player targetPlayer, List<String> args) {
        ...
        try{
            ...
            for (int avatarId: constellationsData.keySet()){
                ...
                //avatar.setCoreProudSkillLevel(0);  #removed method in main gc idk why
                //avatar.recalcStats();
                //avatar.recalcConstellations();

                //avatar.getTalentIdList().addAll(constellationsData.get(avatarId));
				CommandHandler.sendMessage(sender,"cons data: " + constellationsData.get(Collections.max(constellationsData.get(avatarId)) % 100 % 10));
				avatar.forceConstellationLevel(Collections.max(constellationsData.get(avatarId)) % 100 % 10);
                avatar.recalcConstellations();
            ...
            for (int avatarId: skillsData.keySet()) {
                ...
                for (String skillId : skillsData.get(avatarId).keySet()) {
					...
					//avatar.setSkillLevel(level,skillid);
