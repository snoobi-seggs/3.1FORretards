...
public final class SpawnCommand implements CommandHandler {
    ...
    public void execute(Player sender, Player targetPlayer, List<String> args) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            if (gadgetData != null) {
                pos.addY(-3);
				//MonsterData monData = new MonsterData()
				//entity = new EntityMonster(scene, monData, pos, level);
				//entity = new EntityGadget(scene, id, pos, targetPlayer.getRotation());
                entity = new EntityVehicle(scene, targetPlayer, id, 0, pos, GetRandomRotation());// targetPlayer.getRotation());
            }
            if (monsterData != null) {
                entity = new EntityMonster(scene, monsterData, pos, level);
				entity.setFightProperty(FightProperty.FIGHT_PROP_CUR_HP,Float.MAX_VALUE);
				entity.setFightProperty(FightProperty.FIGHT_PROP_CUR_DEFENSE,0f);
            }
	...
	private Position GetRandomRotation() {
		Position rotation = new Position();
		rotation.setX((float) (Math.random() * 360));
		rotation.setY((float) (Math.random() * 360));
		rotation.setZ((float) (Math.random() * 360));
		return rotation;
	}
}
