...
public final class Tools {
    public static void createGmHandbooks() throws Exception {
        ...
        //final Int2IntSortedMap questDescs = new Int2IntRBTreeMap(GameData.getQuestDataMap().int2ObjectEntrySet().stream().collect(Collectors.toMap(e -> (int) e.getIntKey(), e -> (int) e.getValue().getDescTextMapHash())));
		//final Int2IntSortedMap gadgetNames = new Int2IntRBTreeMap(GameData.getGadgetDataMap().int2ObjectEntrySet().stream().collect(Collectors.toMap(e -> (int) e.getIntKey(), e -> (int) e.getValue().getJsonName() )));
		...
		/*for (int section = 0; section < 1; section++) {
            ...
            h.forEach((id, hash) -> {
                ...
                for (int i = 0; i < TextStrings.NUM_LANGUAGES; i++)
					if (t == null) {
						handbookBuilders.get(i).append(sId + null + "\n");
					} else {
						handbookBuilders.get(i).append(sId + t.strings[i] + "\n");
					}
            });
